...
function LsitOfTodos({id, date, title, description, onRemove, setClicked, selectedId,setSelectedId, onEdit}) {
  ...
  return (
    <div>      
        ...
        <label>Date<h3 style={completed ? {textDecoration: "line-through"} : null} >{date}</h3></label>
        <label>Title<h3 style={completed ? {textDecoration: "line-through"} : null}>{title}</h3></label>
        <label>Description <h3 style={completed ? {textDecoration: "line-through"} : null}>{description}</h3></label>
